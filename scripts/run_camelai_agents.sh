...
link_common_directories() {
    if [ ! -d "${REPO_BASEDIR}/genericsuite_asdt/camelai/genericsuite_asdt/utils" ]; then
        ln -sf "${REPO_BASEDIR}/genericsuite_asdt/utils" "${REPO_BASEDIR}/genericsuite_asdt/camelai/genericsuite_asdt/utils"
    fi
    if [ ! -d "${REPO_BASEDIR}/genericsuite_asdt/camelai/genericsuite_asdt/tools" ]; then
        ln -sf "${REPO_BASEDIR}/genericsuite_asdt/tools" "${REPO_BASEDIR}/genericsuite_asdt/camelai/genericsuite_asdt/tools"
    fi
}
...
upgrade() {
    link_common_directories
    poetry add openlit@latest pyyaml@latest 'camel-ai[all]@latest'
}
...
if [ "$ACTION" = "run" ]; then
    ...
    echo ""
    echo "Run the GenericSuite Camel-AI poetry upgrade"
    echo ""
    upgrade
